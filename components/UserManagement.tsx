  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-serif font-semibold text-navy-900">Gestão de Usuários</h2>
          <p className="text-gray-500 text-sm">Controle de acesso e perfis do sistema.</p>
        </div>
        <button 
          onClick={handleAddClick}
          className="flex items-center space-x-2 bg-navy-900 text-white px-4 py-2 rounded-lg hover:bg-navy-800 transition-colors shadow-md"
        >
          <Plus size={18} />
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-navy-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg border-t-4 border-gold-500">
            <div className="flex justify-between items-center p-6 border-b border-gray-100">
              <h3 className="text-xl font-serif font-semibold text-navy-900">
                {editingUser?.id ? 'Editar Usuário' : 'Novo Usuário'}
              </h3>
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-navy-900">
                <X size={24} />
              </button>
            </div>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                ...
                <div className="relative">
                    ...
                    className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-gold-500 focus:border-gold-500"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-gold-500 focus:border-gold-500"
              ...
              <div>
                ...
                <select
                  ...
                  className="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-gold-500 focus:border-gold-500"
                >
                  <option value={UserRole.COLLABORATOR}>Colaborador (Visualização Básica)</option>
                  <option value={UserRole.MANAGER}>Gestor (Controle Operacional)</option>
                  <option value={UserRole.ADMIN}>Administrador do Sistema (Acesso Total)</option>
                </select>
